...
df_cigar_EH <- cigar.to.width(overlap_AC_EH_bt2)
...
final_methyl_sites_AS <- lapply(seq_along(mapped_methyl_sites), function(i) {
  df <- data.frame(pos_rgenome = methyl_sites[[i]],pos_seq = mapped_methyl_sites[[i]],pos_align = aligned_methyl_sites[[i]], width_align = width(overlap_AC_AS_bt2)[i], chr_align = seqnames(overlap_AC_AS_bt2)[i], aligned = methyl_on_aligned[[i]], seq_number = sprintf("seq_%03d", 1:length(aligned_methyl_sites))[i])
...
#ALL Bowtie2
combined_df_bt2 <- bind_rows(AC_methyl_df_bt2, AS_methyl_df_bt2, EH_methyl_df_bt2)
...
#ALL Mini
combined_df_mini <- bind_rows(AC_methyl_df_mini, AS_methyl_df_mini, EH_methyl_df_mini)
...
##bt2
library(ggplot2)
ggplot(combined_df, aes(x = pos_align, fill = species)) +
...
ggplot(subset(combined_df, chr_align == "NC_044048.1"), aes(x = pos_align, fill = species)) +
  geom_histogram(alpha = 0.5, position = "identity") +  
  labs(x = "Position", y = "CpGs",title = "CpGs on Chromosome 1") +
  theme_minimal() +
  theme(strip.text.y = element_text(angle = 0))  

ggplot(subset(combined_df, chr_align == "NC_044048.1" & aligned == TRUE), aes(x = pos_align, fill = species)) +
  geom_histogram(alpha = 0.5, position = "identity") +  
  labs(x = "Position", y = "CpGs",title = "CpGs on Chromosome 1 on SMRs") +
  theme_minimal() +
  theme(strip.text.y = element_text(angle = 0))  

ggplot(subset(combined_df, aligned == TRUE), aes(x = pos_align, fill = species)) +
  geom_histogram(alpha = 0.5, position = "identity") +  
