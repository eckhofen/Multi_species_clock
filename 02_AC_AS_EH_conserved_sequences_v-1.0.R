#### Overview ####
# Conserved sequences will be extracted from the cd-hit files
...
ZF_EH_100_mini <- scanBam("results-data/minimap2/ZF_EH_100_minimap.bam")
ZF_EH_100_bt2 <- scanBam("results-data/bowtie2/ZF_EH_CpG_100bp_bt2_.bam")
ZF_EH_1000_mini <- scanBam("results-data/minimap2/ZF_EH_1000_minimap.bam") #creates list of lists (containing alignment region and other information)

ZF_EH_1000_mini <- readGAlignments("results-data/minimap2/ZF_EH_1000_minimap.bam", use.names = TRUE) #creates another type of object which is similar to granges object
ZF_AC_1000_mini <- readGAlignments("results-data/minimap2/ZF_AC_1000_minimap.bam", use.names = TRUE)
ZF_AS_1000_mini <- readGAlignments("results-data/minimap2/ZF_AS_1000_minimap.bam", use.names = TRUE)

ZF_EH_1000_bt2 <- readGAlignments("results-data/bowtie2/ZF_EH_CpG_1000bp_bt2_.bam", use.names = TRUE)

# compare bowtie2 and minimap same species

shared_ZF_EH_1000 <- ZF_EH_1000_bt2[names(ZF_EH_1000_bt2) %in% names(ZF_EH_1000_mini)]
