...
save(AC_methyl_sites_JM, file = "/workspace/cfngle/results-data/05_shared_methyl_values/AC_methyl_sites_JM.Rdata")
save(AS_methyl_sites_JM, file = "/workspace/cfngle/results-data/05_shared_methyl_values/AS_methyl_sites_JM.Rdata")
save(EH_methyl_sites_JM, file = "/workspace/cfngle/results-data/05_shared_methyl_values/EH_methyl_sites_JM.Rdata")
save(JM_methyl_sites_JM, file = "/workspace/cfngle/results-data/05_shared_methyl_values/JM_methyl_sites_JM.Rdata")
save(ZF_methyl_sites_JM, file = "/workspace/cfngle/results-data/05_shared_methyl_values/ZF_methyl_sites_JM.Rdata")
...
# AC_table <- table(meth_sites_names_tmp_AC %in% AC_meth_data_test)
...
AC_selected_CpGs <- meth_sites_names_tmp_AC %in% AC_meth_data_test
meth_columns_tmp <- sapply(meth_sites_names_tmp_AC[meth_sites_names_tmp_AC %in% AC_meth_data_test], function(x) grep(x, AC_meth_data_test)) %>% 
  as.vector()

AC_meth_values_JM <- AC_meth_data[,meth_columns_tmp]

save(AC_selected_CpGs, file = "/workspace/cfngle/results-data/05_shared_methyl_values/AC_selected_CpGs")

##AS
meth_sites_names_tmp <- paste0(gsub("AS_", "",AS_methyl_sites_JM$Chr), "-", AS_methyl_sites_JM$pos_rgenome)
meth_sites_names_tmp %in% colnames(AS_meth_data)
meth_columns_tmp <- sapply(meth_sites_names_tmp, function(x) grep(x, colnames(AS_meth_data))) %>% 
  as.vector()
