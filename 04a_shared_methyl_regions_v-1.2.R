...
load("AC_AS_EH_JM_ZF_overlaps_bt2.RData")
overlap_seqs_bt2
...
save(all_SMR_b_bt2, file = "/workspace/cfngle/results-data/04_SMRs/all_SMR_b_bt2.Rdata")
...
##ZF
ZF_meth_data <- load("/workspace/cfngle/raw-data/ZF/zzz_methyldata/ZF_methyldata.RData")
...
##AC
write.csv(AC_meth_values, file = paste0(save_dir, "AC_meth_values.csv") )
save(AC_meth_values, file = paste0(save_dir, "AC_meth_values.Rdata"))

##AS
write.csv(AS_meth_values, file = paste0(save_dir, "AS_meth_values.csv") )
save(AS_meth_values, file = paste0(save_dir, "AS_meth_values.Rdata"))

##EH
write.csv(EH_meth_values, file = paste0(save_dir, "EH_meth_values.csv") )
save(EH_meth_values, file = paste0(save_dir, "EH_meth_values.Rdata"))

##JM
write.csv(JM_meth_values, file = paste0(save_dir, "JM_meth_values.csv") )
save(JM_meth_values, file = paste0(save_dir, "JM_meth_values.Rdata"))

##ZF
write.csv(ZF_meth_values, file = paste0(save_dir, "ZF_meth_values.csv") )
save(ZF_meth_values, file = paste0(save_dir, "ZF_meth_values.Rdata"))


#### PCA tests and stat tests ####

##AC
PCA_AC <- prcomp(AC_meth_values[c(-71,-74,-27,-69,-2,-3,-17),],scale = TRUE)
...
##AC
ggplot(PCA_values_AC, aes(x = PC1, y = PC2, color = AC_age_tmp, shape = species)) +
  geom_point(aes(cex = 7)) +
  facet_wrap(~species) +
  scale_color_gradient(low = "blue", high = "red") +
  geom_text(aes(label = 1:nrow(AC_meth_data[c(-71,-74,-27,-69,-2,-3,-17),])), nudge_x = 0.6, nudge_y = 0) +
  theme_classic()
...
ggplot(PCA_values_AS, aes(x = PC1, y = PC2, color = AS_age, shape = species)) +
  ...
  facet_wrap(~species) +
  scale_color_gradient(low = "blue", high = "red") +
  geom_text(aes(label = AS_meth_data$id), nudge_x = 0.6, nudge_y = 0) +
  theme_classic()
...
ggplot(PCA_values_EH, aes(x = PC1, y = PC2, color = EH_age)) +
  geom_point(aes(cex = 7)) +
  scale_color_gradient(low = "blue", high = "red") +
  geom_text(aes(label = EH_meth_data$id), nudge_x = 0.6, nudge_y = 0) +
  theme_classic()
...
AC_meth_values_long$SMR <- as.factor(rep(AC_methyl_sites$SMR[meth_sites_names_tmp %in% AC_meth_data_test], times = length(AC_age))) # indexing is necessary because not all CpGs were able to be extracted from the shared sites due o batch correction
