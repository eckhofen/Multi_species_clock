...
load("01_data/02_external_server_outputs/02_conserved_seq/HS_AC_AS_EH_ZF_overlaps.Rdata")
if (exists("HS_overlap_seqs_bt2")) {
...
# using the overlap of the sequences to get the shared methylation regions (SCMRs)
HS_group_gr_overlap <- do.call(c, HS_gr_overlap_seqs)
HS_SCMR_b <- GenomicRanges::reduce(HS_group_gr_overlap)
names(HS_SCMR_b) <- sprintf("HS_SCMR_b_bt2_%03d", seq_along(HS_SCMR_b))

# Saving file
save(HS_SCMR_b, file = paste0(save_folder, "HS_SCMR.RData"))

#### Methylation extraction ####

## function for extraction
get.methyl.sites <- function(seqs_aligned, species = "undefined", SCMRs = "undefined") {
  ...
  ### C) finding overlaps between sequences and SCMRs
  SCMR_index <- subjectHits(findOverlaps(seqs_aligned, SCMRs))
  ...
  final_methyl_sites_AS <- lapply(seq_along(mapped_methyl_sites), function(i) {
    df <- data.frame(seq_names = cigar_width_df$seq_names[i], 
                     ...
                     SCMR = sprintf("SCMR_%03d", SCMR_index[i]),
                     species = species)
...
AC_methyl_sites <- get.methyl.sites(overlap_HS_AC, species = "AC", SCMRs = HS_SCMR_b)
AS_methyl_sites <- get.methyl.sites(overlap_HS_AS, species = "AS", SCMRs = HS_SCMR_b)
EH_methyl_sites <- get.methyl.sites(overlap_HS_EH, species = "EH", SCMRs = HS_SCMR_b)
ZF_methyl_sites <- get.methyl.sites(overlap_HS_ZF, species = "ZF", SCMRs = HS_SCMR_b)
...
if ("SCMR" %in% colnames(methyl_sites_combined) && !("SCMR" %in% colnames(methyl_sites_combined))) {
  methyl_sites_combined <- dplyr::rename(methyl_sites_combined, SCMR = SCMR)
}
...
# histogram
plot_SCMR_position_hist_all <- ggplot(methyl_sites_combined_nor) +
  geom_histogram(position = "dodge", aes(x = pos_nor, fill = species)) +  
  facet_wrap(~ SCMR, scales = "free") +
  labs(x = "Position", y = "CpGs", title = "Number of CpGs and their genomic position") +
  scale_fill_manual(values = color_species) + 
  theme_bw() +
  theme(axis.text.x = element_blank())  
plot_SCMR_position_hist_all
ggsave(filename = paste0(results_folder, "03_SCMR_position_hist_all.pdf"), plot = plot_SCMR_position_hist_all, width = 10, height = 8)

# lines
plot_SCMR_position_lines_all<- ggplot(methyl_sites_combined_nor) +
  ...
  facet_wrap(~ SCMR, scales = "free_x", ncol = 5) +
...
plot_SCMR_position_lines_all
ggsave(filename = paste0(results_folder, "03_SCMR_position_lines_all.pdf"), plot = plot_SCMR_position_lines_all, width = 10, height = 14)
...
EH_metadata_samples <- read.csv(paste0(raw_private_folder, "hake-samples.txt"), sep = "\t")
