...
nrow(rbind(AC_methyl_sites,AS_methyl_sites,EH_methyl_sites,ZF_methyl_sites))
...
# for ZF only
X_ZF <- ZF_meth_values_selected_train %>% 
  select(-rel_age, -species) %>% 
  as.matrix()

Y_ZF <- ZF_meth_values_selected_train[,"rel_age"]

X_test_ZF <- ZF_meth_values_selected_test %>% 
  select(-rel_age, -species) %>% 
  as.matrix()
Y_test_ZF <- ZF_meth_values_selected_test[,"rel_age"]
...
predictions_cor_test <- cor(predictions_test, all_meth_values_selected_test$rel_age, method = "pearson") %>% 
  round(4)
mse_test <- mean((predictions_test - all_meth_values_selected_test$rel_age)^2) %>% 
  round(4)
mae_test <- mean(abs(predictions_test - all_meth_values_selected_test$rel_age)) %>% 
  round(4)
...
model_plot_test <- ggplot(result_df, aes(color = species)) +
  geom_point(aes(x = age, y = age_predicted), cex = 3) +
  scale_color_manual(values = colpalOI) +
  ylim(0,0.25) +
  xlim(0,0.25) +
  labs(title = "Multispecies age prediction (test data set)", subtitle = paste("R=", predictions_cor_test, "MSE=", mse_test, "MAE=", mae_test)) +
  theme_minimal()


predictions_train <- predict(glm_test, newx= X, s = glm_test$lambda.min)
predictions_cor_train <- cor(predictions_train, all_meth_values_selected_train$rel_age, method = "pearson") %>% 
  round(4)
mse_train <- mean((predictions_train - all_meth_values_selected_train$rel_age)^2) %>% 
  round(4)
mae_train <- mean(abs(predictions_train - all_meth_values_selected_train$rel_age)) %>% 
  round(4)

result_df_train <- data.frame(predictions = predictions_train,
                        rel_age = all_meth_values_selected_train$rel_age,
                        species = all_meth_values_selected_train$species)
colnames(result_df_train) <- c("age_predicted", "age", "species")

model_plot_train <- ggplot(result_df_train, aes(color = species)) +
  geom_point(aes(x = age, y = age_predicted), cex = 3) +
  scale_color_manual(values = colpalOI) +
  ylim(0,0.25) +
  xlim(0,0.25) +
  labs(title = "Multispecies age (relative) prediction (train data set)", subtitle = paste("R=", predictions_cor_train, "MSE=", mse_train, "MAE=", mae_train)) +
  theme_minimal()

model_plot_train + model_plot_test +
  plot_layout(nrow = 1)
