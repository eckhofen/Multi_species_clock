...
## AE only for MLM
plot_AE_MLM_REL <- ggplot(df_AE_MLM_REL, aes(x = 0, y = MLM, pattern = type)) +
  geom_boxplot_pattern(
    position = position_dodge(width = .9),
    outlier.shape = NA,
    pattern_fill = "transparent",
  ...
  scale_fill_manual(values = colpal) +
  scale_pattern_manual(values = c("stripe", "circle")) + 
  labs(y = "Absolute error", x = "", pattern = "Data") +
  theme_bw() + 
  ylim(c(0,.125)) +
  theme(axis.text.x = element_blank())

plot_AE_MLM <- ggplot(df_AE_MLM, aes(x = 0, y = MLM, pattern = type)) +
  geom_boxplot_pattern(
    position = position_dodge(width = .9),
    outlier.shape = NA,
    pattern_fill = "transparent",
  ...
  scale_fill_manual(values = colpal) +
  scale_pattern_manual(values = c("stripe", "circle")) + 
  labs(y = "Absolute error (years)", x = "", pattern = "Data") +
  theme_bw() + 
  ylim(c(0,3.5)) +
  theme(axis.text.x = element_blank())
...
## MLM comparison
mlm_comparison_plot <- mlm_eval_rel$plot_train + mlm_eval_rel$plot_test + plot_AE_MLM_REL +
  mlm_eval_chron$plot_train + mlm_eval_chron$plot_test + plot_AE_MLM +
...
## AE comparison
AE_comparison_plot <- plot_AE_comparison_RE_no_out + plot_AE_comparison_RE_no_out_t +plot_AE_comparison_no_out + plot_AE_comparison_no_out_t +
  plot_layout(nrow = 2, guides = "collect") + 
  plot_annotation(tag_levels = 'a') & 
  theme(plot.tag = element_text(size = 18, face = "bold"))
